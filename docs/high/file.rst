...
文件对象是进入 HDF5 世界的入口。 除下列功能外，每个文件实例也是一个组（Ｇroup）（HDF5文件的根组） :ref:`HDF5 group <group>` representing the `root group`.
...
HDF5 文件和标准的Python文件对象一样工作。它们支持像 r/w/a 这样的标准模式，并且在不再使用时应该关闭。但是，HDF5文件没有“文本”与“二进制”模式的概念(r与rb的区别)。

    >>> f = h5py.File('myfile.hdf5','r')

文件名可以是字节字符串或unicode(中文)字符串。有效的模式包括：:

    ========  ================================================
     r        只读，文件必须存在（默认）
     r+       读/写, 文件必须存在
     w        创建文件, 如果文件已存在，则覆盖原文件
     w- or x  创建文件, 如果文件已存在，则失败
     a        读/写（如果存在），否则创建
    ========  ================================================
